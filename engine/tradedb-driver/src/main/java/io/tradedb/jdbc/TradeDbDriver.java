...
public final class TradeDbDriver implements Driver {
  ...
  public Connection connect(String url, Properties info) throws SQLException {
    ...
    Connection connection =
        new JDBC().connect(SQLITE_PREFIX + options.databasePath(), new Properties());
    if (connection == null) {
      throw new SQLException(
          "sqlite-jdbc did not return a connection for " + options.databasePath());
    }
  ...
  public DriverPropertyInfo[] getPropertyInfo(String url, Properties info) {
    DriverPropertyInfo dataRoot =
        property(
            "dataRoot",
            null,
            "Trade data root directory. Resolves to .db/trade.db or .metadata/trade.db.");
    DriverPropertyInfo dbPath =
        property("dbPath", null, "Explicit path to a trade.db SQLite file.");
    DriverPropertyInfo readOnly =
        property("readOnly", "false", "Enable PRAGMA query_only after compatibility bootstrap.");
    DriverPropertyInfo busyTimeout =
        property("busyTimeoutMs", "30000", "SQLite busy timeout in milliseconds.");
    DriverPropertyInfo bootstrapCompat =
        property(
            "bootstrapCompat",
            "true",
            "Create temporary compatibility views for legacy table names.");
