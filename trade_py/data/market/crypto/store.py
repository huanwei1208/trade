...
def btc_live_pilot_checklist(data_root: str | Path, status_payload: dict[str, Any]) -> dict[str, Any]:
    ...
    publish_audits = list((store.crypto_root / "audit" / "publish").glob("*.json"))
    rollback_audits = list((store.crypto_root / "audit" / "rollback").glob("*.json"))
...
class BtcRunStore:
    def __init__(self, data_root: str | Path) -> None:
        ...
        self.crypto_root = self.data_root / "market" / "crypto"
        self.runs_root = self.crypto_root / "runs" / "btc"
        self.current_path = self.crypto_root / "btc_current.json"
        self.compatibility_path = self.crypto_root / "btc.parquet"

    @contextmanager
    def _exclusive_lock(self):
        self.crypto_root.mkdir(parents=True, exist_ok=True)
        lock_path = self.crypto_root / ".btc-assurance.lock"
    ...
    def _was_published(self, run_id: str, current: dict[str, Any] | None) -> bool:
        if current and run_id in {
            ...
            return True
        audit_root = self.crypto_root / "audit" / "publish"
    ...
    def publish(
        ...
        *,
        ...
        temp_compat = self.crypto_root / f".btc.{result.run_id}.tmp"
        temp_current = self.crypto_root / f".btc_current.{result.run_id}.tmp"
        current_restore = self.crypto_root / f".btc_current.restore.{result.run_id}.tmp"
        ...
        publish_audit_root = self.crypto_root / "audit" / "publish"
        ...
        publish_audit_temp = self.crypto_root / f".btc_publish_audit.{publish_event_id}.tmp"
        ...
        try:
            ...
            if backup_path and backup_path.exists():
                restore = self.crypto_root / ".btc.restore.tmp"
    ...
    def rollback_predecessor(self, *, _lock: bool = True) -> dict[str, Any]:
        ...
        audit_root = self.crypto_root / "audit" / "rollback"
        audit_root.mkdir(parents=True, exist_ok=True)
        audit_path = audit_root / f"{event_time.strftime('%Y%m%dT%H%M%S.%fZ')}-{event_id}.json"
        temp_compat = self.crypto_root / f".btc.rollback.legacy.{event_id}.tmp"
        restore_compat = self.crypto_root / f".btc.rollback.restore.{event_id}.tmp"
        restore_current = self.crypto_root / f".btc_current.rollback.restore.{event_id}.tmp"
        audit_temp = self.crypto_root / f".btc_rollback_audit.{event_id}.tmp"
    ...
    def rollback(self, run_id: str, *, _lock: bool = True) -> dict[str, Any]:
        ...
        previous = current
        temp_compat = self.crypto_root / f".btc.rollback.{run_id}.tmp"
        temp_current = self.crypto_root / f".btc_current.rollback.{run_id}.tmp"
        restore_backup = self.crypto_root / ".btc.rollback.restore.tmp"
        current_restore = self.crypto_root / ".btc_current.rollback.restore.tmp"
