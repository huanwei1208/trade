...
Design-quality governance applies with the `core` and `contract` profiles because
this is an additive public developer CLI contract. It does not change trading,
market-data, DB, parquet, API, frontend, or C++ engine behavior.
...
- **Dependencies:** Reuses the installed native `openspec` CLI and the existing
  read-only design-quality evaluator. No new package or network dependency is added.
