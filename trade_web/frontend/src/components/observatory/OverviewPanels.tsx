import type { ObsCompositeSeries, ObsContext, ObsSingleSeries } from "../../lib/api";
import { humanizeEnum } from "../../lib/format";
import { PanelCard } from "../PanelCard";
import { StatusPill } from "../StatusPill";
import { buildWhatChanged, extractLayers, isPlottable, parseDecimal, readMetricString } from "../../lib/observatory";
...
// a metric is absent we show "—" (honest unknown), not a fabricated 0. The
// "window peak" is explicitly labelled as a window peak, NOT an all-time high.
...
export function MarketSummary({ formalSeries, context }: MarketSummaryProps) {
  const rows = (formalSeries?.rows ?? []).filter(isPlottable);
  const latest = rows.length ? rows[rows.length - 1] : null;
  const latestMetrics = latest?.metrics;

  // Window peak is a WINDOW extreme, never presented as an all-time high.
  const closes = rows.map((r) => parseDecimal(r.close)).filter((v): v is number => v !== null);
  const windowPeak = closes.length ? Math.max(...closes).toLocaleString(undefined, { maximumFractionDigits: 2 }) : null;
  ...
  return (
    <PanelCard title="Market summary" subdued className="obs-market-summary">
      <div className="obs-metric-grid" data-testid="market-summary">
        <Metric label="Latest close" value={latest?.close ?? null} unit={latest?.quote ?? ""} note={latest?.date ? `bar ${latest.date}` : undefined} />
        <Metric label="1D return" value={readMetricString(latestMetrics, "return_1d")} unit="%" window="1 day" />
        <Metric label="7D return" value={readMetricString(latestMetrics, "return_7d")} unit="%" window="7 days" />
        <Metric label="30D return" value={readMetricString(latestMetrics, "return_30d")} unit="%" window="30 days" />
        <Metric
          label="Peak drawdown"
        ...
        />
        <Metric
          label="Window peak close"
          value={windowPeak}
          note="within displayed window — NOT an all-time high"
        />
      </div>
...
export function WhyNotFormal({ context }: WhyNotFormalProps) {
  ...
  const formalPurpose = (context?.purpose_fitness ?? []).find((p) => p.purpose === "formal_system_consumption");
  const strictPurpose = (context?.purpose_fitness ?? []).find((p) => p.purpose === "strict_research");
    ...
    <PanelCard title="Why not formal" subdued className="obs-why-not">
      <div className="obs-why-not__body" data-testid="why-not-formal">
        <div className="obs-why-not__row">
          ...
          <StatusPill
            label={formalPurpose?.allowed ? "allowed" : `blocked: ${humanizeEnum(formalPurpose?.status)}`}
        ...
        <div className="obs-why-not__row">
          ...
          <StatusPill
            label={strictPurpose?.allowed ? "allowed" : `blocked: ${humanizeEnum(strictPurpose?.status)}`}
            ...
            <ul className="obs-kv-list">
              {Object.entries(findings).map(([k, v]) => (
                <li key={k}>
                  ...
                  <span className="obs-kv-val">{typeof v === "object" ? JSON.stringify(v) : String(v)}</span>
                </li>
              ))}
            </ul>
          </div>
...
export function WhatChanged({ composite }: WhatChangedProps) {
  const entries = buildWhatChanged(composite);
            ...
            {entries.map((e, i) => (
              <li key={`${e.kind}-${i}`} className="obs-change-item" data-change-kind={e.kind}>
                <span className="obs-change-icon" aria-hidden="true">
                  {e.kind === "added_dates" ? "+" : e.kind === "removed_dates" ? "−" : e.kind === "revised_dates" ? "◆" : e.kind === "quarantined_dates" ? "◇" : "→"}
                </span>
                <div className="obs-change-body">
                  ...
                  {e.evidenceRefs.length > 0 && (
                    <div className="obs-change-evidence">evidence: {e.evidenceRefs.slice(0, 3).join(", ")}</div>
                  )}
                </div>
              </li>
            ))}
