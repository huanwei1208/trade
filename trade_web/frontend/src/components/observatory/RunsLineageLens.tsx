...
type RunsLineageLensProps = {
  ...
  onRetry?: () => void;
  detail?: ObsRunDetail | null;
  detailLoading?: boolean;
  detailError?: ObservatorySafeError | null;
  onDetailRetry?: () => void;
  diff?: ObsRunDiff | null;
  diffLoading?: boolean;
  diffError?: ObservatorySafeError | null;
  onDiffRetry?: () => void;
    ...
    <div className="obs-runs-lens" data-testid="runs-lens">
      <PanelCard title="Runs" subdued>
                    ...
                    <td>
                      <div className="obs-run-actions">
                        <button
                          ...
                          onClick={() => onSelectRun(r.run_id || null)}
                        >
                          Base
                        </button>
                        <button
                          type="button"
                          className="button button--ghost"
                          onClick={() => onCompareRun(r.run_id || null)}
                        >
                          Compare
                        </button>
                      </div>
        ...
        {runs?.catalog_fingerprint && (
          <div className="obs-runs__fingerprint">
            catalog fingerprint {runs.catalog_fingerprint.slice(0, 16)}
          </div>
        )}
      </PanelCard>
      ...
      {selectedRunId && (
        <PanelCard title={`Run detail · ${selectedRunId}`} subdued>
          {detailLoading && <div className="obs-empty">Loading run detail…</div>}
          {detailError && (
            <ObservatoryErrorState
          ...
          {detail && (
            <div data-testid="run-detail">
              <div className="obs-detail-grid">
                ...
                <Detail
                  label="Rows"
                  value={detail.canonical_rows != null ? String(detail.canonical_rows) : null}
                />
              ...
              <div className="obs-cert-split" data-testid="cert-split">
                <div className="obs-cert-split__col">
                  ...
                  <StatusPill
                    label={`published lifecycle: ${humanizeEnum(detail.lifecycle_state)}`}
                    tone="info"
                    subtle
                  />
                  <p className="obs-lens__hint">
                    Recorded at publish time. Not overwritten by later code changes.
                  </p>
                </div>
                <div className="obs-cert-split__col">
                  ...
                  <p className="obs-lens__hint">
                    A replay mismatch changes compatibility only, never the historical fact.
                  </p>
                </div>
              </div>
                ...
                <div className="obs-artifact-list">
                  ...
                  <ul className="obs-code-list">
                    {(detail.artifact_refs ?? []).map((a) => (
                      <li key={a.name}>
                        <code>{a.name}</code> · {a.sha256?.slice(0, 12)}
                      </li>
                    ))}
                  </ul>
                </div>
      ...
      {selectedRunId && compareRunId && (
        <PanelCard title={`Run diff · ${selectedRunId} → ${compareRunId}`} subdued>
          {diffLoading && <div className="obs-empty">Loading run diff…</div>}
          {diffError && (
            <ObservatoryErrorState
          ...
          {diff && (
            <div data-testid="run-diff">
              <div className="obs-diff-flags">
                <StatusPill
                  label={`code ${diff.code_changed ? "changed" : "same"}`}
                  tone={diff.code_changed ? "warn" : "muted"}
                  subtle
                />
                <StatusPill
                  label={`config ${diff.config_changed ? "changed" : "same"}`}
                  tone={diff.config_changed ? "warn" : "muted"}
                  subtle
                />
                <StatusPill
                  label={`schema ${diff.schema_changed ? "changed" : "same"}`}
                  tone={diff.schema_changed ? "warn" : "muted"}
                  subtle
                />
              </div>
              <div className="obs-diff-grid">
                ...
                <DiffCol
                  title="Changed dates"
                  items={(diff.changed_dates ?? []).map(
                    (c) => `${c.date} (${c.base_close}→${c.compare_close})`,
                  )}
                  icon="◆"
                />
              </div>
