...
- **Project-local GitHub CLI for PRs (MANDATORY)**: Use the official GitHub CLI
  ...
  missing, stop the PR workflow and guide the user to authenticate the
  project-local binary with
  `.cache/github-cli/gh_2.65.0_linux_amd64/bin/gh auth login`. Continue only
  after `.cache/github-cli/gh_2.65.0_linux_amd64/bin/gh auth status` succeeds,
  then retry PR creation after every pushed feature branch.
